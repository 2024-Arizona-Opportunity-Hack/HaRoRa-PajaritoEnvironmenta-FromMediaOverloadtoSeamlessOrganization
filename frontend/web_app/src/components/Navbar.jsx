    ...
    return (
        <nav className="bg-base-100 shadow-md">
            <div className="flex justify-center">
                ...
                <div className="w-full max-w-5xl  px-4 py-4 flex justify-between items-center ">
                    {/* Brand */}
                    <Link
                        to="/"
                    ...
                    <div className="flex items-center space-x-6 ">
                        {isAuthenticated && (
                            <>
                                <Link to="/upload" className="btn btn-outline btn-primary text-md">
                                    Upload
                                </Link>
                                <Link to="/search" className="btn btn-outline btn-primary text-md">
                                    Search
                                </Link>

                                {/* Logout Button */}
                                <button className="btn btn-ghost text-md text-primary" onClick={handleLogoutClick}>
                                    Logout
                                </button>
                            </>
                        )}
                        {/* <select
              className="select select-bordered bg-base-100 text-base-content py-2 px-4 rounded-md shadow-md focus:outline-none text-md"
              value={theme}
              onChange={handleThemeChange}
            >
              <option value="nord">Light Mode</option>
              <option value="coffee">Dark Mode</option>
            </select> */}
